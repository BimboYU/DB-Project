...
class UserModel extends BaseModel {
  ...
  async createUser(userData) {
    ...
    console.log("UserModel.createUser executing with binds:", binds);

    const result = await db.executeQuery(sql, binds, { autoCommit: true });

    if (!result.success) {
      console.error("Database error in createUser:", result.error);
  ...
  async updateLastLogin(userId) {
    const sql =
      "UPDATE USER_ACCOUNT SET LAST_LOGIN = SYSDATE WHERE USER_ID = :userId";
    await db.executeQuery(sql, [userId], { autoCommit: true });
  }

  async updatePassword(userId, newPassword) {
    const sql =
      "UPDATE USER_ACCOUNT SET USER_PASSWORD = :password WHERE USER_ID = :userId";
    await db.executeQuery(sql, [newPassword, userId], { autoCommit: true });
  }
  ...
  async deactivateUser(userId) {
    const sql =
      "UPDATE USER_ACCOUNT SET IS_ACTIVE = 'N' WHERE USER_ID = :userId";
